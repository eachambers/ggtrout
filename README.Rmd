...
Let's go through all the palettes, grouped by (sub)species. In general, you can access and view the palettes using the `ggtrout_palette()` function like so:
...
```{r brookie, message = FALSE, fig.width=1.5, fig.height=2.5}
grid_palettes("brook")
```
...
```{r cuttie, message = FALSE, fig.width=1.5, fig.height=3}
...
```{r rainbow, message = FALSE, fig.width=1.5, fig.height=2.5}
